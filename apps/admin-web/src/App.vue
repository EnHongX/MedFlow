            ...
            <el-col :span="12">
              <div class="config-card">
                ...
                <div class="config-card__body">
                  ...
                  <div class="config-card__desc">预约开始后超过多少分钟未签到，系统显示为"已迟到"</div>
                  <div class="config-card__input">
            ...
            <el-col :span="12">
              <div class="config-card">
                ...
                <div class="config-card__body">
                  ...
                  <div class="config-card__desc">预约开始后超过多少分钟未签到，前台可标记为"爽约"</div>
                  <div class="config-card__input">
                    <el-input-number v-model="clinicConfig.noShowThresholdMinutes" :min="5" :step="5" />
                    <span class="config-card__unit">分钟</span>
                  </div>
                </div>
              </div>
...
async function saveConfig() {
  if (clinicConfig.value.lateThresholdMinutes <= 0) {
    ElMessage.error('迟到阈值必须为正整数');
    return;
  }
  if (clinicConfig.value.noShowThresholdMinutes <= 0) {
    ElMessage.error('爽约阈值必须为正整数');
    return;
  }
  if (clinicConfig.value.lateThresholdMinutes >= clinicConfig.value.noShowThresholdMinutes) {
    ElMessage.error('迟到阈值必须小于爽约阈值');
    return;
  }
  try {
    const res = await fetch(`${API}/config`, {
