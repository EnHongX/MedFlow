...
export class QueueController {
  ...
  async checkin(@Body() body: { appointmentId: string }) {
    return this.prisma.$transaction(async (tx) => {
      // Acquire row lock to serialize against concurrent markNoShow
      await tx.$executeRaw`SELECT id FROM appointments WHERE id = ${body.appointmentId} FOR UPDATE`;
      ...
      // Explicit NO_SHOW check with clear error message
      if (appointment.status === 'NO_SHOW') {
        throw new BadRequestException('该预约已标记爽约，无法签到');
      }
