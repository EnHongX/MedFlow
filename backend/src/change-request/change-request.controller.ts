...
export class ChangeRequestController {
  ...
  async approve(@Param('id') id: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      // Lock the appointment row to serialize with markNoShow / cancel / reschedule
      ...
      if (appointment.status !== 'BOOKED') {
        ...
        await writeLog(tx, { type: 'APPROVE_CHANGE_REQUEST', target: `自动驳回：${cr.type === 'CANCEL' ? '取消' : '改期'}预约${cr.appointmentId}`, role: 'FRONTDESK', result: 'fail', remark: `预约状态已变更（当前：${appointment.status}）` });
    ...
    if (result.autoRejected) {
      throw new BadRequestException('该预约状态已变更（已签到/叫号/接诊中），无法批准');
    }
    return result.data;
  }
